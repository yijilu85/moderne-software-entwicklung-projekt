import type {Appointment, BackendAppointment,} from "@/types/types";
...
export function mapBackendToFrontend(backendData: BackendAppointment): Appointment {
    return {
        id: backendData.id,
...
export const useAppointmentHelpers = () => {
    const delAppointment = async (appointmentId: number, userId: number | undefined, showSnackbar: (message: string, color?: string) => void) => {
        try {
            const confirmDeletion = confirm("Möchten Sie diesen Termin wirklich löschen?");
    ...
    const bookAppointment = async (appointmentId: number, userId: number | undefined, showSnackbar: (message: string, color?: string) => void) => {
        const payload = {
            appointmentId: appointmentId, patientId: userId,
        };
    ...
    const cancelSelectedAppointment = async (appointment: Appointment, userId: number | undefined, showSnackbar: (message: string, color?: string) => void) => {
        if (!appointment.id) {
        ...
        if (!((loggedInUser?.userType === "DOCTOR" && appointment.doctor?.id === loggedInUser?.id) || (loggedInUser?.userType === "PATIENT" && appointment.patient?.id === loggedInUser?.id))) {
        ...
        try {
            const confirmCancellation = confirm("Möchten Sie diesen Termin wirklich stornieren?");
            if (!confirmCancellation) return;

            const payload = {
                appointmentId: appointment.id, userId: loggedInUser?.id as number, // Explicit assertion to number
            };
    ...
    const createAppointment = async (appointment: any, showSnackbar: (message: string, color?: string) => void) => {
        try {
    ...
    return {
        delAppointment, bookAppointment, cancelSelectedAppointment, createAppointment,
    };
};
...
export function checkCanCancelAppointment(selectedEvent: Appointment | undefined) {
    if (!selectedEvent || !selectedEvent.patient) {
        return false;
    }
    const loggedInUser = useUserStore().getLoggedInUser;
    return (selectedEvent.patient.id === loggedInUser?.id || selectedEvent.doctor!.id === loggedInUser?.id);
}
...
export function checkCanSeeAppointment(appointment: Appointment) {
    ...
    if (!appointment.patient?.id) {
        return true;
    } else if (loggedInUser !== null && (appointment.patient?.id === loggedInUser.id || appointment.doctor?.id === loggedInUser.id)) {
        return true;
    }
    return false;
}
